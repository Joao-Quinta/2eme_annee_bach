...
function P=HermiteModif(xev,data)
    X=extand(data(1,:));% liste etendue de la premiere ligne de data
    b=DifferencesDiviseeModif2(data);
...
function b=DifferencesDiviseeModif2(data) 
    ...
    while p < length(X)
       ...
       while k < length(bpp)
          ...
          if p==1
              if X(k)==X(k+1)% i=j
                  % cette fois on doit en plus verifier si zi existe
                  if Z(1,k)
                      bp(1,k)=Z(1,k);
                  else
                      bp(1,k)=0; %si non on renvoie 0
                  end
              else
